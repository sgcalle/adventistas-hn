...
status_types = [("stage", "Stage"), ("done", "Done"),
    ("cancelled", "Cancelled")]
...
class Inquiry(models.Model):
    ...
    school_year_id = fields.Many2one("school_base.school_year",
                                     string="School Year")
    current_grade_level_id = fields.Many2one("school_base.grade_level",
                                             string="Current Grade Level")
    grade_level_id = fields.Many2one("school_base.grade_level",
                                     string="Grade Level of interest",
    ...
    community_street_address = fields.Char(
        string="Community residence address", default="", readonly=False)
    reference_family_1 = fields.Char(string="a) References Families",
                                     default="", readonly=False)
    reference_family_2 = fields.Char(string="b) References Families",
                                     default="", readonly=False)
    ...
    street = fields.Char(string="Street Address", readonly=False,
    ...
    state_tasks = fields.One2many(string="State task",
                                  related="status_id.task_ids")

    # status_type = fields.Char(string="Status Type", related="status_id.name")
    status_type = fields.Selection(string="Status Type",
                                   related="status_id.type_id")
    ...
    sources_id_other = fields.Boolean(string="sources_id_other", readonly=True,
                                      related="sources_id.other")
    ...
    def message_get_suggested_recipients(self):
        ...
        try:
            for inquiry in self:
                if inquiry.email:
                    inquiry._message_add_suggested_recipient(recipients,
    ...
    def force_back(self):
        status_ids_ordered = self.env['adm.inquiry.status'].search([],
                                                                   order="sequence")
        index = 0
    ...
    def move_to_next_status(self):
        ...
        if index < len(status_ids_ordered):
            ...
            if self.status_id.type_id == 'done':
                raise exceptions.except_orm(_('Inquiry completed'),
                                            _('The Inquiry is already done'))
            elif self.status_id.type_id == 'cancelled':
                raise exceptions.except_orm(_('Inquiry cancelled'),
                                            _('The Inquiry cancelled'))
            else:
                self.status_id = next_status
    ...
    def _set_full_name(self):
        self.name = formatting.format_name(self.first_name, self.middle_name,
                                           self.last_name)
    ...
    def create(self, values):
        first_status = \
        self.env['adm.inquiry.status'].search([], order="sequence")[0]
    ...
    def create_new_application(self):
        ...
        if not user_created:
            template_id = self.env.ref(
                'adm.email_template_data_inquiry_accepted')
            # ===========================================================================================================
            # Note:
    ...
    def write(self, values):
        ...
        if "status_id" in values:
            ...
            if (next_status.type_id == "done" and not self.application_id):
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        if msg_dict.get('author_id'):
            defaults.update(
                self._onchange_partner_id_values(msg_dict.get('author_id')))
        ...
        if 'company_id' not in defaults and 'team_id' in defaults:
            defaults['company_id'] = self.env['crm.team'].browse(
...
class AdmissionInquirylanguage(models.Model):
    ...
    language_level_id = fields.Many2one("adm.language.level",
